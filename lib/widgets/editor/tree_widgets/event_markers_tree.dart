...
class _EventMarkersTreeState extends State<EventMarkersTree> {
  ...
  Widget build(BuildContext context) {
    return TreeCardNode(
      ...
      elevation: 1.0,
      children: [for (int i = 0; i < markers.length; i++) _buildMarkerCard(i)],
    );
  }
          ...
          children: [
            Expanded(
              child: Slider(
                ...
                secondaryTrackValue: markers[markerIdx].endWaypointRelativePos
                    ?.toDouble(),
                min: 0.0,
                max: waypoints.length - 1.0,
                label: markers[markerIdx].waypointRelativePos.toStringAsFixed(
                  2,
                ),
                onChangeStart: (value) {
            ...
            children: [
              Expanded(
                child: Slider(
                  ...
                  min: 0.0,
                  max: waypoints.length - 1.0,
                  label: markers[markerIdx].endWaypointRelativePos!
                      .toStringAsFixed(2),
                  onChangeStart: (value) {
                    _sliderChangeStart = value;
                  },
            ...
            onGroupTypeChanged: (value) {
              widget.undoStack.add(
                Change(
                  ...
                  (oldValue) {
                    List<Command> cmds = command.commands;
                    markers[markerIdx].command = Command.fromType(
                      oldValue,
                      commands: cmds,
                    ) as CommandGroup;
                    widget.onPathChangedNoSim?.call();
