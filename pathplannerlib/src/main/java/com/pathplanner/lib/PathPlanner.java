...
public class PathPlanner {
   ...
   *
   * @param name The name of the path group to load
   * @param constraints A List of PathConstraints (max velocity, max acceleration) of the paths in
   *     the group. If there are less constraints than paths, the last constrain given will be used
  ...
  public static List<PathPlannerTrajectory> loadPathGroup(
    ...
    if (constraints == null || constraints.isEmpty()) {
      throw new IllegalArgumentException(
          "You must specify at least one PathConstraints object in order to load your path group.");
    }
    return loadPathGroup(
        name,
        false,
        constraints.get(0),
