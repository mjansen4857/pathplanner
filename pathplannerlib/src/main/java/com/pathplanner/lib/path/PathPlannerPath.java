  ...
  public PathPlannerPath mirrorPath() {
    if (isChoreoPath) {
      ...
      for (PathPlannerTrajectory.State state : choreoTrajectory.getStates()) {
        ...
        mirrored.positionMeters = GeometryUtil.mirrorTranslation(state.positionMeters);
        mirrored.heading = state.heading.unaryMinus();
        mirrored.targetHolonomicRotation = state.targetHolonomicRotation.unaryMinus();
        mirrored.holonomicAngularVelocityRps = state.holonomicAngularVelocityRps;
        if (mirrored.holonomicAngularVelocityRps.isPresent()) {
          mirrored.holonomicAngularVelocityRps =
              Optional.of(-mirrored.holonomicAngularVelocityRps.get());
        }
    ...
    List<Translation2d> newBezier =
        bezierPoints.stream().map(GeometryUtil::mirrorTranslation).collect(Collectors.toList());
    List<RotationTarget> newRotTargets =
        rotationTargets.stream()
            .map(
                (t) ->
                    new RotationTarget(
                        t.getPosition(), t.getTarget().unaryMinus(), t.shouldRotateFast()))
            .collect(Collectors.toList());
    GoalEndState newEndState =
        new GoalEndState(goalEndState.getVelocity(), goalEndState.getRotation().unaryMinus());
    Rotation2d newPreviewRot = previewStartingRotation.unaryMinus();
