...
public record PointTowardsZone(String name, Translation2d targetPosition, Rotation2d rotationOffset, double minPosition, double maxPosition) {
    ...
    static PointTowardsZone fromJson(JSONObject zoneJson) {
        ...
        Translation2d targetPos = JSONUtil.translation2dFromJson((JSONObject) zoneJson.get("fieldPosition"));
        Rotation2d rotationOffset = Rotation2d.fromDegrees(((Number) zoneJson.get("rotationOffset")).doubleValue());
    ...
    public PointTowardsZone flip() {
        return new PointTowardsZone(name, FlippingUtil.flipFieldPosition(targetPosition), rotationOffset, minPosition, maxPosition);
    }
}
