...
public class PathPlannerTrajectoryState implements Interpolatable<PathPlannerTrajectoryState> {
    ...
    public PathPlannerTrajectoryState interpolate(PathPlannerTrajectoryState endVal, double t) {
        ...
        lerpedState.timeSeconds = MathUtil.lerp(timeSeconds, endVal.timeSeconds, t);
        double deltaT = lerpedState.timeSeconds - timeSeconds;
        if (deltaT < 0) {
            return endVal.interpolate(this, 1 - t);
        }
        lerpedState.fieldSpeeds = new ChassisVelocities(MathUtil.lerp(fieldSpeeds.vx, endVal.fieldSpeeds.vx, t), MathUtil.lerp(fieldSpeeds.vy, endVal.fieldSpeeds.vy, t), MathUtil.lerp(fieldSpeeds.omega, endVal.fieldSpeeds.omega, t));
        lerpedState.heading = heading;
        lerpedState.linearVelocity = MathUtil.lerp(linearVelocity, endVal.linearVelocity, t);
        ...
        lerpedState.pose = new Pose2d(lerpedXPos, lerpedYPos, pose.getRotation().interpolate(endVal.pose.getRotation(), t));
    ...
    public PathPlannerTrajectoryState reverse() {
        ...
        Translation2d reversedSpeeds = new Translation2d(fieldSpeeds.vx, fieldSpeeds.vy).rotateBy(Rotation2d.k180deg);
        reversed.fieldSpeeds = new ChassisVelocities(reversedSpeeds.getX(), reversedSpeeds.getY(), fieldSpeeds.omega);
