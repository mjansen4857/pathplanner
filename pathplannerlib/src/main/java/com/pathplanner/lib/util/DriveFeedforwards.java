 ...
 *     feedforwards due to friction forces.
 */
public record DriveFeedforwards(double[] accelerationsMPSSq, double[] linearForcesNewtons, double[] torqueCurrentsAmps, double[] robotRelativeForcesXNewtons, double[] robotRelativeForcesYNewtons) implements Interpolatable<DriveFeedforwards> {
     ...
     *     to friction forces.
     */
    public DriveFeedforwards(LinearAcceleration[] accelerations, Force[] linearForces, Current[] torqueCurrents, Force[] robotRelativeForcesX, Force[] robotRelativeForcesY) {
        this(Arrays.stream(accelerations).mapToDouble(x -> x.in(MetersPerSecondPerSecond)).toArray(), Arrays.stream(linearForces).mapToDouble(x -> x.in(Newtons)).toArray(), Arrays.stream(torqueCurrents).mapToDouble(x -> x.in(Amps)).toArray(), Arrays.stream(robotRelativeForcesX).mapToDouble(x -> x.in(Newtons)).toArray(), Arrays.stream(robotRelativeForcesY).mapToDouble(x -> x.in(Newtons)).toArray());
    }
    ...
    public static DriveFeedforwards zeros(int numModules) {
        return new DriveFeedforwards(new double[numModules], new double[numModules], new double[numModules], new double[numModules], new double[numModules]);
    }

    @Override
    public DriveFeedforwards interpolate(DriveFeedforwards endValue, double t) {
        return new DriveFeedforwards(interpolateArray(accelerationsMPSSq, endValue.accelerationsMPSSq, t), interpolateArray(linearForcesNewtons, endValue.linearForcesNewtons, t), interpolateArray(torqueCurrentsAmps, endValue.torqueCurrentsAmps, t), interpolateArray(robotRelativeForcesXNewtons, endValue.robotRelativeForcesXNewtons, t), interpolateArray(robotRelativeForcesYNewtons, endValue.robotRelativeForcesYNewtons, t));
    }
    ...
    public DriveFeedforwards reverse() {
        if (accelerationsMPSSq.length != 2) {
            throw new IllegalStateException("Feedforwards should only be reversed for differential drive trains");
        }
        return new DriveFeedforwards(new double[] { -accelerationsMPSSq[1], -accelerationsMPSSq[0] }, new double[] { -linearForcesNewtons[1], -linearForcesNewtons[0] }, new double[] { -torqueCurrentsAmps[1], -torqueCurrentsAmps[0] }, new double[] { -robotRelativeForcesXNewtons[1], -robotRelativeForcesXNewtons[0] }, new double[] { -robotRelativeForcesYNewtons[1], -robotRelativeForcesYNewtons[0] });
    }
    ...
    public DriveFeedforwards flip() {
        return new DriveFeedforwards(FlippingUtil.flipFeedforwards(accelerationsMPSSq), FlippingUtil.flipFeedforwards(linearForcesNewtons), FlippingUtil.flipFeedforwards(torqueCurrentsAmps), FlippingUtil.flipFeedforwardXs(robotRelativeForcesXNewtons), FlippingUtil.flipFeedforwardYs(robotRelativeForcesYNewtons));
    }
    ...
    public LinearAcceleration[] accelerations() {
        return Arrays.stream(accelerationsMPSSq).mapToObj(MetersPerSecondPerSecond::of).toArray(LinearAcceleration[]::new);
    }
