...
public class PPLibTelemetry {
    ...
    private static final DoubleArrayPublisher velPub = NetworkTableInstance.getDefault().getDoubleArrayTopic("/PathPlanner/vel").publish();

    private static final StructPublisher<Pose2d> posePub = NetworkTableInstance.getDefault().getStructTopic("/PathPlanner/currentPose", Pose2d.struct).publish();

    private static final StructArrayPublisher<Pose2d> pathPub = NetworkTableInstance.getDefault().getStructArrayTopic("/PathPlanner/activePath", Pose2d.struct).publish();

    private static final StructPublisher<Pose2d> targetPosePub = NetworkTableInstance.getDefault().getStructTopic("/PathPlanner/targetPose", Pose2d.struct).publish();
    ...
    private static void ensureHotReloadListenersInitialized() {
        if (hotReloadPathListener == null) {
            hotReloadPathListener = NetworkTableListener.createListener(NetworkTableInstance.getDefault().getStringTopic("/PathPlanner/HotReload/hotReloadPath"), EnumSet.of(NetworkTableEvent.Kind.VALUE_REMOTE), PPLibTelemetry::handlePathHotReloadEvent);
        }
        if (hotReloadAutoListener == null) {
            hotReloadAutoListener = NetworkTableListener.createListener(NetworkTableInstance.getDefault().getStringTopic("/PathPlanner/HotReload/hotReloadAuto"), EnumSet.of(NetworkTableEvent.Kind.VALUE_REMOTE), PPLibTelemetry::handleAutoHotReloadEvent);
        }
    }
    ...
    private static void handlePathHotReloadEvent(NetworkTableEvent event) {
        if (!compMode) {
            ...
            try {
                ...
                if (RobotBase.isReal()) {
                    File pathFile = new File(Filesystem.getDeployDirectory(), "pathplanner/paths/" + name + ".path");
                    try (FileWriter writer = new FileWriter(pathFile)) {
                        writer.write(pathJson.toJSONString());
                        writer.flush();
                    } catch (IOException e) {
                        DriverStationErrors.reportWarning("Failed to save updated path file contents, please re-deploy code", false);
                    }
                }
            } catch (Exception e) {
                // Ignore
    ...
    private static void handleAutoHotReloadEvent(NetworkTableEvent event) {
        if (!compMode) {
            ...
            try {
                ...
                if (RobotBase.isReal()) {
                    File pathFile = new File(Filesystem.getDeployDirectory(), "pathplanner/autos/" + name + ".auto");
                    try (FileWriter writer = new FileWriter(pathFile)) {
                        writer.write(autoJson.toJSONString());
                        writer.flush();
                    } catch (IOException e) {
                        DriverStationErrors.reportWarning("Failed to save updated auto file contents, please re-deploy code", false);
                    }
                }
            } catch (Exception e) {
                // Ignore
