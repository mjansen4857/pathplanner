...
void FollowPathWithEvents::Execute()
{
    ...
    if (m_unpassedMarkers.size() > 0 && currentTime >= m_unpassedMarkers[0].time)
    {
        ...
        for (std::string name : marker.names)
        {
            ...
            {
                FRC_ReportError(frc::warn::Warning, "PathPlanner attempted to schedule an event missing from the event map: " + name);
            }
        }
    }
}
