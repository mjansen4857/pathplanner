...
bool PPHolonomicDriveController::atReference(){
...
frc::ChassisSpeeds PPHolonomicDriveController::calculate(frc::Pose2d& currentPose, PathPlannerTrajectory::PathPlannerState& referenceState){
    ...
    units::meters_per_second_t xFeedback = units::meters_per_second_t(this->m_xController.Calculate(currentPose.X().value(), referenceState.pose.X().value()));
    units::meters_per_second_t yFeedback = units::meters_per_second_t(this->m_yController.Calculate(currentPose.Y().value(), referenceState.pose.Y().value()));
    units::radians_per_second_t rotationFeedback = units::radians_per_second_t(this->m_rotationController.Calculate(currentPose.Rotation().Radians().value(), referenceState.holonomicRotation.Radians().value()));

    return frc::ChassisSpeeds::FromFieldRelativeSpeeds(xFF + xFeedback, yFF + yFeedback, rotationFF + rotationFeedback, currentPose.Rotation());
}