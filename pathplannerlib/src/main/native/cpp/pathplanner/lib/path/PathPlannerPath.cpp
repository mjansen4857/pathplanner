...
frc::Translation2d PathPlannerPath::mirrorTranslation(frc::Translation2d translation) {
	return frc::Translation2d{translation.X(), FlippingUtil::fieldSizeY - translation.Y()};
}
...
std::shared_ptr<PathPlannerPath> PathPlannerPath::mirrorPath() {
    ...
    if (m_idealTrajectory.has_value()) {
        ...
        for (const auto& s : traj.getStates()) {
            ...
            state.pose = frc::Pose2d(
                mirrorTranslation(s.pose.Translation()),
                -s.pose.Rotation()
            );
            state.fieldSpeeds = frc::ChassisSpeeds(
                s.fieldSpeeds.vx,
                -s.fieldSpeeds.vy,
                -s.fieldSpeeds.omega
            );
            ...
            if (ff.accelerations.size() == 4) {
                ...
                state.feedforwards = DriveFeedforwards{
                    std::vector<units::meters_per_second_squared_t>{ff.accelerations[1], ff.accelerations[0]},
                    std::vector<units::newton_t>{ff.linearForces[1], ff.linearForces[0]},
                    std::vector<units::ampere_t>{ff.torqueCurrents[1], ff.torqueCurrents[0]},
                    std::vector<units::newton_t>{ff.robotRelativeForcesX[1], ff.robotRelativeForcesX[0]},
                    std::vector<units::newton_t>{ff.robotRelativeForcesY[1], ff.robotRelativeForcesY[0]}
				};
            } else {
    ...
    auto path = std::make_shared<PathPlannerPath>(m_globalConstraints, m_goalEndState);
    ...
    for (auto& z : m_pointTowardsZones) {
        newPointTowardsZones.emplace_back(PointTowardsZone{
            z.getName(),
            mirrorTranslation(z.getTargetPosition()),
            z.getRotationOffset(),
            z.getMinWaypointRelativePos(),
            z.getMaxWaypointRelativePos()}
        );
    }
    ...
    path->m_goalEndState = GoalEndState(
        m_goalEndState.getVelocity(),
        -m_goalEndState.getRotation()
    );
