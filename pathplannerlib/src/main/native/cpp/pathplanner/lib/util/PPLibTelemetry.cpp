...
// std::unordered_map<std::string,
// 			std::vector<std::shared_ptr<PathPlannerAuto>>> PPLibTelemetry::m_hotReloadAutos = std::unordered_map<std::string,
...
// void PPLibTelemetry::registerHotReloadAuto(std::string autoName, std::shared_ptr<PathPlannerAuto> ppAuto){
// 	if(!m_compMode){
// 		PPLibTelemetry::ensureHotReloadListenersInitialized();
//
// 		if(m_hotReloadAutos.find(autoName) == m_hotReloadAutos.end()){
// 			m_hotReloadAutos.emplace(autoName, std::vector<std::shared_ptr<PathPlannerAuto>>());
// 		}
//
