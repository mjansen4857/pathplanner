...
SwerveSetpointGenerator::SwerveSetpointGenerator(const RobotConfig &config,
		wpi::units::turns_per_second_t maxSteerVelocity) : m_robotConfig(
		config), maxSteerVelocity(maxSteerVelocity) {
	//	brownoutVoltage = wpi::RobotController::GetBrownoutVoltage();
...
SwerveSetpoint SwerveSetpointGenerator::generateSetpoint(
	...
	if (std::isnan(inputVoltage.value())) {
		...
		inputVoltage = wpi::units::math::max(inputVoltage, brownoutVoltage);
	}
	...
	if (constraints.has_value()) {
		wpi::math::Translation2d vel(wpi::units::meter_t {
				desiredStateRobotRelative.vx() }, wpi::units::meter_t {
				desiredStateRobotRelative.vy() });
		wpi::units::meters_per_second_t linearVel { vel.Norm()() };
		if (linearVel > constraints.value().getMaxVelocity()) {
			vel = vel * (constraints.value().getMaxVelocity()() / linearVel());
		}
		auto angVel = wpi::units::math::max(
				wpi::units::math::min(desiredStateRobotRelative.omega,
						constraints.value().getMaxAngularVelocity()),
				-constraints.value().getMaxAngularVelocity());
		desiredStateRobotRelative = wpi::math::ChassisVelocities(
				wpi::units::meters_per_second_t { vel.X()() },
				wpi::units::meters_per_second_t { vel.Y()() }, angVel);
	}

	std::vector < wpi::math::SwerveModuleVelocity > desiredModuleStates =
			m_robotConfig.toSwerveModuleVelocitys(desiredStateRobotRelative);
	// Make sure desiredState respects velocity limits.
	desiredModuleStates = m_robotConfig.desaturateWheelSpeeds(
			desiredModuleStates, maxSpeed);
	desiredStateRobotRelative = m_robotConfig.toChassisVelocities(
			desiredModuleStates);
	...
	if (epsilonEquals(desiredStateRobotRelative,
	...
	if (all_modules_should_flip
		...
		return generateSetpoint(prevSetpoint, wpi::math::ChassisVelocities(),
				constraints, dt, inputVoltage);
	}
	...
	wpi::units::meters_per_second_t dx = desiredStateRobotRelative.vx
			- prevSetpoint.robotRelativeSpeeds.vx;
	wpi::units::meters_per_second_t dy = desiredStateRobotRelative.vy
			- prevSetpoint.robotRelativeSpeeds.vy;
	wpi::units::radians_per_second_t dtheta = desiredStateRobotRelative.omega
	...
	// lazy).
	std::vector < std::optional < wpi::math::Rotation2d >> overrideSteering;
	...
	for (size_t m = 0; m < m_robotConfig.numModules; m++) {
		...
		wpi::units::radian_t max_theta_step = dt * maxSteerVelocity;

		if (epsilonEquals(prevSetpoint.moduleStates[m].velocity.value(), 0.0)) {
			...
			if (epsilonEquals(desiredModuleStates[m].velocity.value(), 0.0)) {
				// Goal angle doesn't matter. Just leave module at its current angle.
				overrideSteering[m] = prevSetpoint.moduleStates[m].angle;
				continue;
			}

			wpi::math::Rotation2d neccesarryRotation =
			...
			double numStepsNeeded = wpi::units::math::abs(
		...
		// friction force.
		wpi::units::radian_t maxHeadingChange { (dt.value()
				* m_robotConfig.wheelFrictionForce.value())
				/ ((m_robotConfig.mass.value() / m_robotConfig.numModules)
						* wpi::units::math::abs(
								prevSetpoint.moduleStates[m].velocity).value()) };
		max_theta_step = wpi::units::math::min(max_theta_step,
				maxHeadingChange);
	...
	wpi::math::Translation2d chassisForceVec;
	wpi::units::newton_meter_t chassisTorque { 0.0 };
	for (size_t m = 0; m < m_robotConfig.numModules; m++) {
		wpi::units::radians_per_second_t lastVelRadPerSec {
				(prevSetpoint.moduleStates[m].velocity
		...
		wpi::units::ampere_t currentDraw =
				m_robotConfig.moduleConfig.driveMotor.Current(
						wpi::units::math::abs(lastVelRadPerSec), inputVoltage);
		wpi::units::ampere_t reverseCurrentDraw = wpi::units::math::abs(
				m_robotConfig.moduleConfig.driveMotor.Current(
						wpi::units::math::abs(lastVelRadPerSec),
						-inputVoltage));
		currentDraw = wpi::units::math::min(currentDraw,
				m_robotConfig.moduleConfig.driveCurrentLimit);
		currentDraw = wpi::units::math::max(currentDraw, 0_A);
		reverseCurrentDraw = wpi::units::math::min(reverseCurrentDraw,
				m_robotConfig.moduleConfig.driveCurrentLimit);
		reverseCurrentDraw = wpi::units::math::max(reverseCurrentDraw, 0_A);
		wpi::units::newton_meter_t forwardModuleTorque =
				m_robotConfig.moduleConfig.driveMotor.Torque(currentDraw);
		wpi::units::newton_meter_t reverseModuleTorque =
				m_robotConfig.moduleConfig.driveMotor.Torque(
						reverseCurrentDraw);

		wpi::units::meters_per_second_t prevSpeed =
				prevSetpoint.moduleStates[m].velocity;
		desiredModuleStates[m] = desiredModuleStates[m].Optimize(
				prevSetpoint.moduleStates[m].angle);
		wpi::units::meters_per_second_t desiredSpeed =
				desiredModuleStates[m].velocity;

		int forceSign;
		wpi::math::Rotation2d forceAngle = prevSetpoint.moduleStates[m].angle;
		wpi::units::newton_meter_t moduleTorque;
		if (epsilonEquals(prevSpeed.value(), 0)
			...
			if (prevSpeed < 0_mps) {
				forceAngle = forceAngle + wpi::math::Rotation2d(180_deg);
			}
		} else {
		...
		wpi::units::newton_t forceAtCarpet = moduleTorque
				/ m_robotConfig.moduleConfig.wheelRadius;
		wpi::math::Translation2d moduleForceVec = { ((forceAtCarpet * forceSign)
	...
	wpi::math::Translation2d chassisAccelVec = chassisForceVec
			/ m_robotConfig.mass.value();
	wpi::units::radians_per_second_squared_t chassisAngularAccel {
			chassisTorque.value() / m_robotConfig.MOI.value() };

	if (constraints.has_value()) {
		wpi::units::meters_per_second_squared_t linearAccel {
		...
		chassisAngularAccel = wpi::units::math::max(
				wpi::units::math::min(chassisAngularAccel,
						constraints.value().getMaxAngularAcceleration()),
				-constraints.value().getMaxAngularAcceleration());
	}

	// Use kinematics to convert chassis accelerations to module accelerations
	wpi::math::ChassisVelocities chassisAccel { chassisAccelVec.X() / 1_s,
			chassisAccelVec.Y() / 1_s, chassisAngularAccel * 1_s };
	std::vector < wpi::math::SwerveModuleVelocity > accelStates =
			m_robotConfig.toSwerveModuleVelocitys(chassisAccel);
	...
	for (size_t m = 0; m < m_robotConfig.numModules; m++) {
		...
		wpi::units::meters_per_second_t maxVelStep = wpi::units::math::abs(
				accelStates[m].velocity * dt.value());

		wpi::units::meters_per_second_t vx_min_s =
				min_s == 1.0 ?
						desired_vx[m] :
						(desired_vx[m] - prev_vx[m]) * min_s + prev_vx[m];
		wpi::units::meters_per_second_t vy_min_s =
				min_s == 1.0 ?
						desired_vy[m] :
	...
	wpi::math::ChassisVelocities retSpeeds = {
	...
	wpi::units::meters_per_second_t prevVelX =
			prevSetpoint.robotRelativeSpeeds.vx;
	wpi::units::meters_per_second_t prevVelY =
			prevSetpoint.robotRelativeSpeeds.vy;
	wpi::units::meters_per_second_squared_t chassisAccelX = (retSpeeds.vx
			- prevVelX) / dt;
	wpi::units::meters_per_second_squared_t chassisAccelY = (retSpeeds.vy
			- prevVelY) / dt;
	wpi::units::newton_t chassisForceX = chassisAccelX * m_robotConfig.mass;
	wpi::units::newton_t chassisForceY = chassisAccelY * m_robotConfig.mass;

	wpi::units::radians_per_second_squared_t angularAccel = (retSpeeds.omega
			- prevSetpoint.robotRelativeSpeeds.omega) / dt;
	wpi::units::newton_meter_t angTorque { angularAccel.value()
			* m_robotConfig.MOI.value() };
	wpi::math::ChassisVelocities chassisForces { chassisForceX * 1_s / 1_kg,
			chassisForceY * 1_s / 1_kg, wpi::units::radians_per_second_t {
					angTorque.value() } };

	std::vector < wpi::math::Translation2d > wheelForces =
			m_robotConfig.chassisForcesToWheelForceVectors(chassisForces);

	std::vector < wpi::math::SwerveModuleVelocity > retStates =
			m_robotConfig.toSwerveModuleVelocitys(chassisForces);
	std::vector < wpi::units::meters_per_second_squared_t
			> accelFF(m_robotConfig.numModules);
	std::vector < wpi::units::newton_t
			> linearForceFF(m_robotConfig.numModules);
	std::vector < wpi::units::ampere_t
			> torqueCurrentFF(m_robotConfig.numModules);
	std::vector < wpi::units::newton_t > forceXFF(m_robotConfig.numModules);
	std::vector < wpi::units::newton_t > forceYFF(m_robotConfig.numModules);
	for (size_t m = 0; m < m_robotConfig.numModules; m++) {
		...
		std::optional < wpi::math::Rotation2d > maybeOverride =
				overrideSteering[m];
		if (maybeOverride.has_value()) {
			wpi::math::Rotation2d override = maybeOverride.value();
			if (flipHeading((-retStates[m].angle).RotateBy(override))) {
				retStates[m].velocity = -retStates[m].velocity;
				appliedForce = -appliedForce;
				torqueCurrent = -torqueCurrent;
			}
			retStates[m].angle = override;
		}
		wpi::math::Rotation2d deltaRotation =
				(-prevSetpoint.moduleStates[m].angle).RotateBy(
						retStates[m].angle);
		...
		accelFF[m] = (retStates[m].velocity
				- prevSetpoint.moduleStates[m].velocity) / dt;
...
SwerveSetpoint SwerveSetpointGenerator::generateSetpoint(
	...
	return generateSetpoint(prevSetpoint, desiredStateRobotRelative,
			constraints, dt, wpi::units::volt_t {
					wpi::RobotController::GetInputVoltage() });
}

double SwerveSetpointGenerator::findSteeringMaxS(
		wpi::units::meters_per_second_t x_0,
		wpi::units::meters_per_second_t y_0, wpi::units::radian_t theta_0,
		wpi::units::meters_per_second_t x_1,
		wpi::units::meters_per_second_t y_1, wpi::units::radian_t theta_1,
		wpi::units::radian_t max_deviation) {
	theta_1 = unwrapAngle(theta_0.value(), theta_1.value());
	wpi::units::radian_t diff = theta_1 - theta_0;
	if (wpi::units::math::abs(diff) < max_deviation) {
		// Can go all the way to s=1.
		return 1.0;
	}

	wpi::units::radian_t target = theta_0
			+ wpi::units::math::copysign(max_deviation, diff);
	...
	double sin = wpi::units::math::sin(-target);
	double cos = wpi::units::math::cos(-target);
...
double SwerveSetpointGenerator::findDriveMaxS(
		wpi::units::meters_per_second_t x_0,
	...
	double target = sqrt_l_0
			+ wpi::units::math::copysign(max_vel_step, diff).value();
