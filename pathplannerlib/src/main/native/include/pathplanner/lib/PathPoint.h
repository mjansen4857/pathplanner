...
public:
	...
	constexpr PathPoint(frc::Translation2d position, frc::Rotation2d heading,
			frc::Rotation2d holonomicRotation) : PathPoint(position, heading,
			holonomicRotation, -1_mps) {
	}

	constexpr PathPoint(frc::Translation2d position, frc::Rotation2d heading,
			units::meters_per_second_t velocityOverride) : PathPoint(position,
			heading, frc::Rotation2d(), velocityOverride) {
	}

	constexpr PathPoint(frc::Translation2d position, frc::Rotation2d heading) : PathPoint(
			position, heading, frc::Rotation2d(), -1_mps) {
	}

	static PathPoint fromCurrentHolonomicState(frc::Pose2d currentPose,
			frc::ChassisSpeeds currentSpeeds) {
		units::meters_per_second_t linearVel = units::math::sqrt(
	...
	constexpr static PathPoint fromCurrentDifferentialState(
			frc::Pose2d currentPose, frc::ChassisSpeeds currentSpeeds) {
